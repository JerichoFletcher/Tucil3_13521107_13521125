...
namespace PathfindAllDayTest {
    ...
    public class AlgorithmTests {
        ...
        public void TestUCS() {
            ...
            // UCS instance construction test
            GraphTraversalAlgorithm<GraphNode<char>> ucs = new GraphTraversalAlgorithm<GraphNode<char>>(graph, true) {
                GFunction = info => info.ExpandNode.GCost + info.ExpandEdge.Data,
                HFunction = info => 0d
            };
            ...
            expectedPath = new GraphNode<char>[] { node1, node2, node4 };
            foundPath = ucs.FindPath(node1, node4);
            Assert.IsTrue(Enumerable.SequenceEqual(expectedPath, foundPath));

            expectedPath = new GraphNode<char>[] { node6, node2, node4 };
            foundPath = ucs.FindPath(node6, node4);
            Assert.IsTrue(Enumerable.SequenceEqual(expectedPath, foundPath));

            expectedPath = new GraphNode<char>[] { node1, node3, node5 };
            foundPath = ucs.FindPath(node1, node5);
            Assert.IsTrue(Enumerable.SequenceEqual(expectedPath, foundPath));

            expectedPath = new GraphNode<char>[] { node3 };
            foundPath = ucs.FindPath(node3, node3);
            Assert.IsTrue(Enumerable.SequenceEqual(expectedPath, foundPath));

            // Path search fail test
            GraphNode<char> node7 = new GraphNode<char>('G', 0f, 0f);
            Assert.ThrowsException<ArgumentException>(() => ucs.FindPath(node1, node7));

            Assert.IsNull(ucs.FindPath(node1, node6));
            Assert.IsNull(ucs.FindPath(node6, node3));
            Assert.IsNull(ucs.FindPath(node3, node2));
        }
    }
}
